...
function t1SwitchPipe(mode) {
    ...
    if (mode === 'trend') {
        ...
        if (searchArea) {
            ...
            if (marketPanel) searchArea.appendChild(marketPanel);
            if (subtabBar) {
                searchArea.appendChild(subtabBar);
                subtabBar.style.display = 'flex'; // 검색에선 표시 !
                subtabBar.style.marginBottom = '20px';
            }
            if (grid) searchArea.appendChild(grid);
            if (pagination) searchArea.appendChild(pagination);
        }
...
function t1FeedRenderMarketPanel(keyword, rawData, wholesaleItems) {
    ...
    html += `<div class="mp-header" style="display:flex; flex-direction:column; gap:12px;">
        <div style="display:flex; justify-content:space-between; align-items:center;">
            <div>
                <span class="mp-title">🛒 "${escapeHtml(keyword)}" 네이버 시장 분석</span>
                <span class="mp-count" style="margin-left:8px;">${items.length}개 상품 벤치마킹</span>
            </div>
            <button class="mp-close" onclick="document.getElementById('t1-market-panel').style.display='none'" title="접기">✕</button>
        </div>
        
        <!-- 🔥 시장 수요 인텔리전스 보드 -->
        <div style="display:flex; gap:16px; background:rgba(255,255,255,0.05); padding:12px 16px; border-radius:8px; align-items:center;">
            <div style="display:flex; flex-direction:column; min-width:100px;">
                <span style="font-size:11px; color:#94a3b8; margin-bottom:4px;">월간 총 검색량</span>
                <span style="font-size:18px; font-weight:700; color:#fff;">${fmt(monthlySearch)}<span style="font-size:12px; font-weight:normal; color:#94a3b8; margin-left:2px;">회</span></span>
            </div>
            <div style="width:1px; height:30px; background:rgba(255,255,255,0.1)"></div>
            <div style="display:flex; flex-direction:column; min-width:100px;">
                <span style="font-size:11px; color:#94a3b8; margin-bottom:4px;">시장 추정 전환율(CVR)</span>
                <span style="font-size:18px; font-weight:700; color:${estimatedCvr >= 5.0 ? '#34d399' : '#fff'};">${estimatedCvr.toFixed(1)}<span style="font-size:12px; font-weight:normal; margin-left:2px;">%</span></span>
            </div>
            <div style="width:1px; height:30px; background:rgba(255,255,255,0.1)"></div>
            <div style="display:flex; flex-direction:column; min-width:100px;">
                <span style="font-size:11px; color:#94a3b8; margin-bottom:4px;">리뷰 급상승 트렌드</span>
                <span style="font-size:18px; font-weight:700; color:${reviewSurge >= 30 ? '#f43f5e' : '#fff'};">+${reviewSurge}<span style="font-size:12px; font-weight:normal; color:#94a3b8; margin-left:2px;">건/일</span></span>
            </div>
            <div style="flex:1;"></div>
            ${isHot ? `<div style="background:rgba(244,63,94,0.1); border:1px solid #f43f5e; color:#f43f5e; padding:6px 14px; border-radius:30px; font-size:12px; font-weight:bold; letter-spacing:0.5px; animation: pulseRed 2s infinite;">🔥 Sourcing Recommended</div>` : ''}
        </div>
    </div>`;

    // 가격 요약 3칸
    html += `<div class="mp-prices" style="margin-top:0px;">
        <div class="mp-price-card mp-low">
            <div class="mp-price-val">${fmt(minP)}</div>
            <div class="mp-price-label">최저가</div>
            <div class="mp-price-sub">${lowItems.length}개 상품</div>
        </div>
        <div class="mp-price-card mp-avg">
            <div class="mp-price-val">${fmt(avgP)}</div>
            <div class="mp-price-label">평균가</div>
            <div class="mp-price-sub">중위 ${fmt(medP)}</div>
        </div>
        <div class="mp-price-card mp-high">
            <div class="mp-price-val">${fmt(maxP)}</div>
            <div class="mp-price-label">최고가</div>
            <div class="mp-price-sub">${highItems.length}개 상품</div>
        </div>
    </div>`;
