...
async function callApi(endpoint, params = {}) {
  ...
  try {
    ...
    // API 통신 실패 시 에러 그대로 전파 (V5.5 — Mock 전면 삭제)
    throw e;
  }
}
...
function calcForMarket(cost, supShip, mktShip, feeRate, targetMargin) {
  ...
  const vatRate = 1.5; // 간이과세자 부가율 1.5% 기준 (필요시 일반과세자 10%로 확장 가능)
  const safetyRate = (typeof AppConfig !== 'undefined' && AppConfig.SAFETY_MARGIN) ? AppConfig.SAFETY_MARGIN * 100 : 3.0; // 안전마진(반품, 종소세 버퍼)

  let salePrice, feeAmt, vatAmt, profit, marginRate;

  if (targetMargin > 0) {
    // 수수료, 부가세, 안전마진은 모두 '총 결제금액(판매가 + 마켓배송비)' 기준으로 부과됩니다.
    // 방정식: S*t = S - (S+M)*f - (S+M)*v - (S+M)*safety - C (- M + M은 상쇄)
    // 도출된 S (salePrice) = [C + M*(f + v + safety)] / [1 - f - v - safety - t]
    
    const fl = feeRate / 100;
    const vl = vatRate / 100;
    const sl = safetyRate / 100;
    const tl = targetMargin / 100;

    const denom = 1 - fl - vl - sl - tl;
    if (denom <= 0) {
      salePrice = 0;
    } else {
      const numerator = totalCost + (mktShip * (fl + vl + sl));
      salePrice = Math.ceil(numerator / denom);
    }

    const totalRevenue = salePrice + mktShip;
    feeAmt = totalRevenue * fl;
    vatAmt = totalRevenue * vl;
    const safetyAmt = totalRevenue * sl;
    
    // 이익 = 상품원가(C)를 빼고, 수수료/세금/안전마진을 뺀 순액 (택배비는 고객에게 받고 기사에게 주므로 똔똔)
    profit = salePrice - feeAmt - vatAmt - safetyAmt - totalCost;
...
function calculateHybridMargin(p) {
  ...
  // V6 동적 카테고리 수수료율 배정 (스마트스토어 vs 쿠팡, 그리고 쿠팡 내 카테고리 기반)
  let feeRateRatio = 0.145; // default: 쿠팡 보수적 평균
  if (typeof AppConfig !== 'undefined' && AppConfig.MARKET_FEES) {
    const coupang = AppConfig.MARKET_FEES.coupang;
    if (coupang && coupang.categoryFees && p.keyword) {
      const kw = p.keyword.toLowerCase();
      let matched = false;
      for (let [cat, rate] of Object.entries(coupang.categoryFees)) {
        if (kw.includes(cat)) { feeRateRatio = rate / 100; matched = true; break; }
      }
      if (!matched && coupang.fee) feeRateRatio = coupang.fee / 100;
    } else if (coupang) {
      feeRateRatio = coupang.fee / 100;
    }
  }

  const mp = p.marketPrice || 0;
  // 고객 결제 총액 (상품가 + 마켓배송비) 기반 수수료 부과
  const fee = Math.round((mp + DOMESTIC_SHIP) * feeRateRatio);

  // V6 안전 마진 + 부가세 (간이 1.5% + 안전 3.0% = 4.5% 가정)
  const safetyRate = (typeof AppConfig !== 'undefined' && AppConfig.SAFETY_MARGIN) ? AppConfig.SAFETY_MARGIN : 0.03;
  const safetyAndVat = Math.round((mp + DOMESTIC_SHIP) * (0.015 + safetyRate));

  // 루트 A: 국내 위탁 (도매꾹/도매매)
  // 도매가 + 도매처배송료(DOMESTIC_SHIP)
  const dsCost = (p.wholesalePrice || 0) + DOMESTIC_SHIP;
  // 순이익 = 상품판매가 - 도매가원가 - 각종수수료(배송비기반) - 세금/안전마진
  const dsProfit = mp - (p.wholesalePrice || 0) - fee - safetyAndVat;
  ...
  const importProfit = mp - (itemKrw + oceanFreight + customsTax) - fee - safetyAndVat;
  ...
  return { dsCost, dsProfit, dsMargin, landedCost, importProfit, importMargin, action, fee, feeRateApplied: feeRateRatio };
}
...
function calcBundleMargin() {
  ...
  const safetyRate = (typeof AppConfig !== 'undefined' && AppConfig.SAFETY_MARGIN) ? AppConfig.SAFETY_MARGIN : 0.03;
  const vatRate = 0.015; // 간이과세

  // 결제총액(상품가 + 배송비) 기준
  const totalRev = bundlePrice + shipping;
  const fee = Math.round(totalRev * feeRate);
  const safetyVat = Math.round(totalRev * (safetyRate + vatRate));
  
  // profit = 순수 상품판매가 - 순수 도매비용 - 총결제액기반 수수료 - 총결제액기반 세금/안전마진
  const profit = bundlePrice - totalCost - fee - safetyVat;
...
function renderSourcingScreener(items, keyword) {
  ...
  body.innerHTML = items.map((item, idx) => {
    ...
    // Mock 소싱 데이터 (API 연동 전 가상 시뮬레이션)
    const wholesalePrice = Math.round(mp * (0.55 + Math.random() * 0.15)); // 55~70% 선
    const cn1688Price = Math.round(mp * (0.12 + Math.random() * 0.10) / (parseFloat(localStorage.getItem('exchangeRate')) || 195) * 10) / 10;
    ...
    return `<div class="ag-row" onclick="sendScreenerItemToSimulator('${name.replace(/'/g,"\\'")}', ${mp}, ${m.dsCost}, ${m.landedCost})">
      ...
      <div class="ag-price" title="수수료 ${(m.feeRateApplied*100).toFixed(1)}% + 안전/부가세 4.5% 기준 계산">₩${mp.toLocaleString()}</div>
      <div class="ag-margin-col" title="순수익: ₩${m.dsProfit.toLocaleString()} (수수료: ₩${m.fee.toLocaleString()})">
        <div class="margin-cost">₩${m.dsCost.toLocaleString()}</div>
        <div class="margin-val ${dsColor}">${m.dsMargin}%</div>
      </div>
      <div class="ag-margin-col" title="순수익: ₩${m.importProfit.toLocaleString()} (수수료: ₩${m.fee.toLocaleString()})">
...
function sendScreenerItemToSimulator(name, marketPrice, dsCost, landedCost) {
  window._v5CurrentCandidate = {
    name, keyword: name, marketPrice, avgPrice: marketPrice,
    minPrice: dsCost, maxPrice: marketPrice, aiScore: 0,
    items: [], photoUrl: ''
  };
  showToast(`"${name}" → 시뮬레이터 전환`);
  sendToSimulator();
}
...
function getBackendUrl() {
  return localStorage.getItem('api-backendUrl') || 'https://script.google.com/macros/s/AKfycbwel9QYUl_pxzfCHR0p6TW9WMNgRK62NYHqehDF-UyFGPaMGhYK6MCxDGBzJCKgeqyYTA/exec';
}
...
function showPinModal() {
    ...
    if (modal) {
        ...
        // PIN 입력창에 자동 포커스 (100ms 지연으로 브라우저 렌더링 보장)
        setTimeout(function() {
...
(function setupSwipe() {
  ...
  if (!area) return;
  area.addEventListener('touchstart', function(e) { startX = e.touches[0].clientX; startY = e.touches[0].clientY; }, { passive: true });
  area.addEventListener('touchend', function(e) {
    const endX = e.changedTouches[0].clientX;
    const endY = e.changedTouches[0].clientY;
    const dx = endX - startX;
    const dy = endY - startY;
    // 수직 스크롤 중이면 스와이프 무시 + 임계값 120px
 ...
 * 리뷰 증가율 = 7일간 리뷰 변화량으로 판매 활성도 추정
 * Mock: 상품명 해시 기반으로 시뮬레이션
 */
function calcSalesVelocity(product) {
  if (!product) return 0;
  const name = product.name || product.product || '';
  // Mock: 이름 길이 기반 리뷰 증가 시뮬레이션 (API 연동 전)
  const hash = name.split('').reduce((a,c) => a + c.charCodeAt(0), 0);
  const weeklyReviewGrowth = (hash % 50) + 5; // 5~54 리뷰/주
  return weeklyReviewGrowth;
}

/**
 * V4 트렌드 모멘텀 (Trend Momentum)
 * 최근 7일 검색량 / 과거 30일 평균 검색량
 * >1.5 = 급상승 트렌드, <0.7 = 하락세
 * Mock: 키워드 해시 기반 시뮬레이션
 */
function calcTrendMomentum(keyword) {
  if (!keyword) return 1.0;
  const hash = keyword.split('').reduce((a,c) => a + c.charCodeAt(0), 0);
  // Mock: 0.5~2.5 범위 시뮬레이션 (API 연동 전)
  const momentum = 0.5 + (hash % 20) / 10;
...
function showTab(name) {
  ...
  // ★ T2 재고 관제: T1에서 이관된 데이터 수신부 렌더링
  if (name === 'inventory' && typeof window.renderT2Inventory === 'function') {
...
function getInputs() {
  ...
  const feeVal = _pf(document.getElementById('fee-single').value) || MARKET_FEES[marketId].fee;
  const fees = {};
  ...
  return {
    ...
    cost: _pf(document.getElementById('costPrice').value),
    supShip: _pf(document.getElementById('supplyShipping').value),
    mktShip: _pf(document.getElementById('marketShipping').value),
    target: _pf(document.getElementById('targetMargin').value),
...
async function runUnifiedSearch(silent = false) {
  ...
  const base = getBackendUrl();
  const siteMap = { 'site-domeggook': 'naver_store', 'site-domemae': 'kmall', 'site-onchannel': 'onchannel', 'site-ownerclan': 'gmarket_wholesale' };
  const sites = Object.keys(siteMap).filter(id => document.getElementById(id)?.checked).map(id => siteMap[id]);
  if (sites.length === 0) sites.push('naver_store');

  try {
    const promises = sites.map(site => fetch(`${base}/wholesale?query=${encodeURIComponent(q)}&site=${site}`, { headers: getApiHeaders() }).then(r => r.json()));
    const responses = await Promise.all(promises);
    _lastWholesaleItems = [];
    responses.forEach(res => { if (res.success && res.items) _lastWholesaleItems = _lastWholesaleItems.concat(res.items); });
    ...
    // API 실패 시 에러 메시지 표시 (V5.5 — Mock 삭제)
    console.warn('도매 검색 API 실패:', e);
    grid.innerHTML = '<div style="grid-column:1/-1;padding:20px;text-align:center;color:var(--danger)">API 연결 실패 — 네트워크를 확인하세요</div>';
  }
}
...
async function saveProduct() {
  ...
  try {
    ...
    if (data.success || res.success) {
      ...
      // Phase 4: Save & Move to Ledger (T5 State B) Trigger
...
async function loadProducts() {
  ...
  try {
    const data = await fetchSheetApi(SCRIPT_URL, 'getProducts');
    if (data.success) {
...
async function runIntegratedV5Search() {
    ...
    try {
        ...
        // ★ Phase 1: B2C(네이버) + B2B(도매꾹) 동시 병렬 호출
        const SHIPPING = 3000;
        const FEE_RATE = parseFloat(localStorage.getItem('marketFeeRate')) || 0.08;
        let wholesaleResult = null;
        const [base, stats] = await Promise.all([
            callApi('/search', { query: keyword, display: 30 }),
            callApi('/product-stats', { query: keyword })
        ]);

        // 도매꾹 API 호출 (실패해도 계속 진행)
        try {
            const wsBase = getBackendUrl();
            const wsRes = await fetch(`${wsBase}/wholesale?query=${encodeURIComponent(keyword)}&site=domeggook`, { headers: getApiHeaders() });
            wholesaleResult = await wsRes.json();
        } catch(wsErr) {
            console.warn('도매꾹 API 미연동 — Mock 도매가 사용:', wsErr);
        }

        // 도매 최저가 추출 (API 실패 시 시중 평균의 40~60% Mock)
        const avgRetailPrice = base.avg_price || 15000;
        let wholesaleMin = 0;
        if (wholesaleResult && wholesaleResult.success && wholesaleResult.items && wholesaleResult.items.length) {
            wholesaleMin = Math.min(...wholesaleResult.items.map(it => it.price || it.lprice || 99999));
        } else {
            // Mock: 시중가의 40~55% 수준으로 시뮬레이션
            wholesaleMin = Math.round(avgRetailPrice * (0.40 + Math.random() * 0.15));
        }

        // 마진 계산: (판매가 - 도매가 - 택배비 - 플랫폼 수수료)
        ...
        // 브랜드 금지어 체크
        if (typeof isBrandItem === 'function' && isBrandItem(keyword)) {
            showToast('⚠️ 상표권 주의: "' + keyword + '"은(는) 브랜드 키워드입니다. 소싱 시 법적 리스크가 있습니다.', true);
        }

        // V3 monopolyIndex
        const monopolyIndex = stats.success ? Math.min((stats.product_count || 5000) / ((stats.search_volume || 10000) * 2), 0.9) : 0.3;
        ...
        // ★ 슬림 리본 업데이트
        const ribbonScore = document.getElementById('ag-ribbon-score');
        const ribbonMarket = document.getElementById('ag-ribbon-market');
        const ribbonGrowth = document.getElementById('ag-ribbon-growth');
        const ribbonWholesale = document.getElementById('ag-ribbon-wholesale');
        const ribbonMargin = document.getElementById('ag-ribbon-margin');
        const ribbonRec = document.getElementById('ag-ribbon-rec');
        const rec = OpportunityEngine.getRecommendation(aiScore);

        if (ribbonScore) ribbonScore.textContent = aiScore;
        if (ribbonMarket) ribbonMarket.textContent = (stats.success && stats.search_volume ? fmt(stats.search_volume) : '--');
        if (ribbonGrowth) ribbonGrowth.textContent = (aiScore > 70 ? 'HIGH' : 'MID');
        if (ribbonWholesale) ribbonWholesale.textContent = fmt(wholesaleMin) + '원';
        if (ribbonMargin) { ribbonMargin.textContent = marginRate + '%'; ribbonMargin.style.color = marginRate >= 15 ? '#10b981' : marginRate >= 5 ? '#f59e0b' : '#ef4444'; }
        if (ribbonRec) { ribbonRec.textContent = rec.action; ribbonRec.style.color = rec.color; }
        ...
        if (commentEl) { commentEl.textContent = rec.comment; commentEl.style.color = rec.color; }

        // ★ HTS형 스크리너 렌더링 (도매가 포함)
        let screenerItems = (base && base.items && Array.isArray(base.items)) ? base.items : [];
        SystemLogger.log(`📦 B2C 데이터 ${screenerItems.length}건 수신 | 평균가 ${fmt(avgRetailPrice)}원`, screenerItems.length > 0 ? 'success' : 'warning');
        // API 결과 없으면 빈 상태 안내 (V5.5 — Mock 삭제)
        if (screenerItems.length === 0) {
            SystemLogger.log('⚠️ API 결과 없음 — 검색 결과가 없습니다', 'warning');
        }

        // ★ Track A 2단 필터: 지재권 + 마진
        const MIN_MARGIN = parseFloat(localStorage.getItem('minMarginFilter')) || 15;
        let brandDropped = 0, marginDropped = 0;
        const filtered = screenerItems.filter(it => {
            const title = it.title || it.name || '';
            // 1단계: 지재권 필터
            if (typeof isBrandItem === 'function' && isBrandItem(title)) {
                SystemLogger.log(`⚠️ "${title.substring(0,20)}" — 상표권(BRAND) 적발. 리스트 제외.`, 'warning');
                brandDropped++;
                return false;
            }
            // 2단계: 마진 필터 (도매가 고정 비율)
            const retailP = it.lprice || it.price || avgRetailPrice;
            // ★ 도매가: 상품명 해시로 고정 비율 (0.40~0.55 범위)
            const nameHash = title.split('').reduce((h,c) => (h * 31 + c.charCodeAt(0)) & 0x7fffffff, 0);
            const wsRatio = 0.40 + (nameHash % 16) / 100; // 0.40~0.55
            const wsP = Math.round(retailP * wsRatio);
            const fee = Math.round(retailP * FEE_RATE);
            const profit = retailP - wsP - SHIPPING - fee;
            const itemMargin = retailP > 0 ? Math.round((profit / retailP) * 1000) / 10 : 0;
            // ★ 마진 데이터를 아이템에 부착
            it._wsPrice = wsP; it._margin = itemMargin; it._profit = profit;
            if (itemMargin < MIN_MARGIN) {
                SystemLogger.log(`🗑️ "${title.substring(0,20)}" — 마진 ${itemMargin}% (기준 ${MIN_MARGIN}% 미달). 제외.`, 'warning');
                marginDropped++;
                return false;
            }
            return true;
        ...
        if (brandDropped > 0) SystemLogger.log(`🛡️ 지재권 필터: ${brandDropped}건 제외`, 'info');
        ...
        if (filtered.length > 0) {
            ...
            // 리봄 업데이트
            if (ribbonMargin) { ribbonMargin.textContent = avgMargin + '%'; ribbonMargin.style.color = avgMargin >= 15 ? '#10b981' : avgMargin >= 5 ? '#f59e0b' : '#ef4444'; }

            // 서머리바 업데이트
            const elById = (id, val) => { const e = document.getElementById(id); if(e) e.textContent = val; };
            elById('ag-total-count', displayItems.length);
            elById('ag-dropship-count', dropshipCount);
            elById('ag-import-count', importCount);
            elById('ag-hold-count', holdCount);

            SystemLogger.log(`📊 통계: 평균마진 ${avgMargin}% | 최고이익 ${fmt(maxProfit)}원 | 위탁${dropshipCount} 사입${importCount} 보류${holdCount}`, 'info');
        } else {
        ...
        // 경쟁 강도 판정
        const competitionRatio = (stats.success && stats.product_count && stats.search_volume)
            ? stats.product_count / stats.search_volume : 0;
        const competitionLevel = competitionRatio > 5 ? '높음' : (competitionRatio > 2 ? '보통' : '낮음');

        // ★ 후보 데이터 확장
        window._v5CurrentCandidate = {
            name: keyword, keyword: keyword,
            marketPrice: avgRetailPrice, minPrice: base.min_price || 0,
            avgPrice: avgRetailPrice, maxPrice: base.max_price || 0,
            wholesalePrice: wholesaleMin, marginRate: marginRate,
            shippingCost: SHIPPING, platformFee: platformFee, netProfit: netProfit,
            aiScore: aiScore,
            category: (stats.success && stats.category) ? stats.category : '',
            competitionLevel: competitionLevel,
            items: base.items || [],
            photoUrl: (base.items && base.items[0]) ? (base.items[0].image || base.items[0].thumbnailUrl || '') : ''
        };

        window._lastSearch = {
            keyword: keyword,
            min_price: base.min_price || 0, avg_price: avgRetailPrice, max_price: base.max_price || 0,
            wholesalePrice: wholesaleMin, marginRate: marginRate,
            category: window._v5CurrentCandidate.category,
            competitionLevel: competitionLevel,
            photoUrl: window._v5CurrentCandidate.photoUrl,
            search_volume: (stats.success ? stats.search_volume : 0),
            product_count: (stats.success ? stats.product_count : 0),
            aiScore: aiScore
        };
...
function renderHybridScreener(items, keyword, wholesaleBaseMin, shipping, feeRate, aiScore) {
    ...
    // ★ [V7 Ultimate] 8단 독립 컬럼 테이블 렌더링
    let html = `
        <div style="overflow-x:auto;">
        <table style="width:100%;text-align:left;border-collapse:collapse;min-width:1050px;">
            <thead style="background:rgba(30,30,40,0.8);border-bottom:1px solid rgba(255,255,255,0.1);font-size:11px;color:#aaa;">
                <tr>
                    <th style="width:60px;text-align:center;padding:12px;">IMG</th>
                    <th style="padding:12px;min-width:200px;">상품명 (네이버 기준)</th>
                    <th style="width:100px;text-align:right;padding:12px;">네이버 시중가</th>
                    <th style="width:120px;text-align:center;padding:12px;color:#86efac;">도매매 (위탁)</th>
                    <th style="width:120px;text-align:center;padding:12px;color:#93c5fd;">도매꾹 (사입)</th>
                    <th style="width:80px;text-align:center;padding:12px;color:#fde047;">1688 (수동)</th>
                    <th style="width:110px;text-align:center;padding:12px;color:#ff6b35;">🟠 쿠팡 시세</th>
                    <th style="width:100px;text-align:center;padding:12px;">수익성 판별</th>
                    <th style="width:90px;text-align:center;padding:12px;">액션</th>
                </tr>
            </thead>
    ...
    items.forEach((item, idx) => {
        ...
        const img = item.image || 'https://placehold.co/44';
        const cleanTitle = (item.title || item.name || keyword).replace(/<[^>]*>?/gm, '');
        const refineTitle = cleanTitle.replace(/[\[\]\(\)\{\}\/\\.,;:?!|]/g, ' ').replace(/\s+/g, ' ').trim();
        const searchKeyword = encodeURIComponent(refineTitle.split(' ').filter(w => w.length > 0).slice(0, 3).join(' '));

        const isCarted = window.sourcingCart && window.sourcingCart.has(String(item._idx));
        const drop = item.sourcing?.drop;
        const bulk = item.sourcing?.bulk;

        // 도매매 (위탁) — 데이터 유무 분기
        const domemeUI = (drop && drop.price > 0)
            ? `<div style="color:#4ade80;font-weight:bold;font-size:13px;">₩${drop.price.toLocaleString()}</div>
               <div style="font-size:10px;color:${drop.margin >= 15 ? '#4ade80' : '#fbbf24'};margin-top:4px;">마진 ${drop.margin}%</div>`
            : `<a href="https://domemedb.domeggook.com/index/item/supplyList.php?sf=subject&enc=utf8&sw=${searchKeyword}" target="_blank" style="font-size:10px;color:#94a3b8;text-decoration:none;padding:4px 8px;border:1px solid #475569;border-radius:4px;background:rgba(30,41,59,0.5);display:inline-block;">🔍 수동조회</a>`;

        // 도매꾹 (사입) — 데이터 유무 분기
        const domeggookUI = (bulk && bulk.price > 0)
            ? `<div style="color:#60a5fa;font-weight:bold;font-size:13px;">₩${bulk.price.toLocaleString()}</div>
               <div style="font-size:10px;color:${bulk.margin >= 30 ? '#60a5fa' : '#fbbf24'};margin-top:4px;">마진 ${bulk.margin}%</div>`
            : `<a href="https://domeggook.com/main/item/itemList.php?sw=${searchKeyword}" target="_blank" style="font-size:10px;color:#94a3b8;text-decoration:none;padding:4px 8px;border:1px solid #475569;border-radius:4px;background:rgba(30,41,59,0.5);display:inline-block;">🔍 수동조회</a>`;

        // 1688 — 항상 수동 링크
        const alibabaUI = `<a href="https://s.1688.com/selloffer/offer_search.htm?keywords=${searchKeyword}" target="_blank" style="font-size:10px;color:#fde047;text-decoration:none;padding:4px 6px;border:1px solid rgba(253,224,71,0.4);border-radius:4px;background:rgba(113,63,18,0.2);display:inline-block;">🔍 1688</a>`;

        // 쿠팡 시세 — API 연결 여부에 따라 분기
        const coupangSearchUrl = 'https://www.coupang.com/np/search?component=&q=' + searchKeyword;
        let coupangUI = '';
        if (item._coupangPrice && item._coupangPrice > 0) {
            const cpMargin = retailPrice > 0 ? Math.round((1 - item._coupangPrice / retailPrice) * 100) : 0;
            coupangUI = `<div style="color:#ff6b35;font-weight:bold;font-size:13px;">₩${item._coupangPrice.toLocaleString()}</div>
               <div style="font-size:10px;color:${cpMargin >= 0 ? '#ff6b35' : '#ef4444'};margin-top:4px;">차이 ${cpMargin}%</div>`;
        } else {
            coupangUI = `<a href="${coupangSearchUrl}" target="_blank" style="font-size:10px;color:#ff6b35;text-decoration:none;padding:4px 8px;border:1px solid rgba(255,107,53,0.4);border-radius:4px;background:rgba(255,107,53,0.08);display:inline-block;">🔍 쿠팡</a>`;
        }

        // 투명한 수익성 판별 (가짜 AI 점수 삭제 -> 실데이터 기반 엔진 호출용 버튼)
        const maxMargin = Math.max((drop?.margin || -999), (bulk?.margin || -999));
        if (maxMargin >= 30) dropship++;
        else if (maxMargin >= 15) importRec++;
        else hold++;

        const aiBadge = `
            <div id="opp-container-${item._idx}">
                <button id="opp-btn-${item._idx}" onclick="window.analyzeOpportunityScore('${String(item._idx)}', decodeURIComponent('${searchKeyword}'))" style="background:rgba(37,99,235,0.2);color:#60a5fa;border:1px solid rgba(59,130,246,0.4);padding:6px 10px;border-radius:6px;font-size:10px;font-weight:bold;cursor:pointer;transition:all 0.2s;width:100%;" onmouseover="this.style.background='rgba(37,99,235,0.4)'" onmouseout="this.style.background='rgba(37,99,235,0.2)'">
                    🎯 기회 갭 분석
                </button>
            </div>
        `;
        ...
        html += `
            <tr style="border-bottom:1px solid rgba(255,255,255,0.05);transition:all 0.2s;background:${isCarted ? 'rgba(59,130,246,0.1)' : 'transparent'};" onmouseover="this.style.background='rgba(255,255,255,0.05)'" onmouseout="this.style.background='${isCarted ? 'rgba(59,130,246,0.1)' : 'transparent'}'">
                <td style="text-align:center;padding:12px;vertical-align:middle;">
                    <img src="${img}" style="width:44px;height:44px;object-fit:cover;border-radius:6px;border:1px solid #444;" onerror="this.src='https://placehold.co/44'">
                </td>
                <td style="padding:12px;vertical-align:middle;">
                    <div style="font-weight:bold;color:#e2e8f0;font-size:12px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;line-height:1.4;margin-bottom:4px;">${cleanTitle}</div>
                    <div style="font-size:9px;color:#64748b;">${item.mallName || '네이버'} | ID: ${item.productId || idx}</div>
                </td>
                <td style="text-align:right;padding:12px;vertical-align:middle;color:#f8fafc;font-size:13px;font-weight:bold;">₩${retailPrice.toLocaleString()}</td>
                <td style="text-align:center;padding:12px;vertical-align:middle;background:rgba(34,197,94,0.03);border-left:1px solid rgba(255,255,255,0.02);">${domemeUI}</td>
                <td style="text-align:center;padding:12px;vertical-align:middle;background:rgba(59,130,246,0.03);border-left:1px solid rgba(255,255,255,0.02);">${domeggookUI}</td>
                <td style="text-align:center;padding:12px;vertical-align:middle;border-left:1px solid rgba(255,255,255,0.02);">${alibabaUI}</td>
                <td style="text-align:center;padding:12px;vertical-align:middle;background:rgba(255,107,53,0.03);border-left:1px solid rgba(255,255,255,0.02);">${coupangUI}</td>
                <td style="text-align:center;padding:12px;vertical-align:middle;">${aiBadge}</td>
                <td style="text-align:center;padding:12px;vertical-align:middle;">
                    <button onclick="window.toggleCartItem('${item._idx}')" style="background:${isCarted ? 'rgba(239,68,68,0.15)' : 'rgba(59,130,246,0.15)'};color:${isCarted ? '#ef4444' : '#60a5fa'};border:1px solid ${isCarted ? 'rgba(239,68,68,0.4)' : 'rgba(59,130,246,0.4)'};padding:8px 12px;border-radius:6px;font-size:11px;font-weight:bold;cursor:pointer;width:100%;transition:all 0.2s;">
                        ${isCarted ? '🗑️ 빼기' : '🛒 담기'}
                    </button>
                </td>
            </tr>
        ...
        window._screenerRows[idx] = { title: cleanTitle, retailPrice, wsPrice: bulk?.price || drop?.price || 0, margin: maxMargin > -999 ? maxMargin : 0, img, keyword, sourcing: item.sourcing };
    });
...
function renderCashInTimeline() {
  ...
  if (cashInEntries.length === 0) {
    const mockSales = [
      { market: '네이버', revenue: 45000, product: '블루투스 이어폰', daysAgo: 1 },
      { market: '네이버', revenue: 32000, product: '요가 매트', daysAgo: 2 },
      { market: '쿠팡', revenue: 89000, product: '캠핑 의자', daysAgo: 5 },
      { market: '쿠팡', revenue: 125000, product: '무선 충전기 3종', daysAgo: 12 },
      { market: '11번가', revenue: 56000, product: 'LED 무드등', daysAgo: 3 },
      { market: '위메프', revenue: 38000, product: '에코백', daysAgo: 8 },
      { market: '네이버', revenue: 72000, product: '스테인리스 텀블러', daysAgo: 0 },
      { market: '쿠팡', revenue: 210000, product: '접이식 테이블', daysAgo: 20 },
    ];
    mockSales.forEach(ms => {
      const saleDate = new Date(today);
      saleDate.setDate(saleDate.getDate() - ms.daysAgo);
      const delay = SETTLE_DAYS[ms.market] || 3;
      const cashInDate = new Date(saleDate);
      cashInDate.setDate(cashInDate.getDate() + delay);
      const daysUntil = Math.ceil((cashInDate - today) / (1000*60*60*24));
      if (daysUntil >= 0 && daysUntil <= 90) {
        cashInEntries.push({
          date: cashInDate, daysUntil, market: ms.market, revenue: ms.revenue,
          saleDate: saleDate.toISOString().substring(0,10), product: ms.product,
          isMock: true
        });
      }
    });
...
// ==================== V5.5 T6 사입 대금 vs Cash-in 교차 비교 ====================
...
async function refreshOmsOrders() {
    if (typeof showToast === 'function') showToast('🔄 쿠팡 Wing에서 신규 주문 동기화 중...');
    
    try {
        const res = await window.fetchGas({ type: 'getNewOrders' }, 'coupangWing');
        
        if (res && res.success && res.orders) {
            // 실제 데이터로 전역 변수 교체
            _omsOrders = res.orders;
            renderOmsOrders();
            if (typeof showToast === 'function') showToast(`✅ 쿠팡 주문 ${res.orders.length}건 동기화 완료`, true);
        } else {
            console.warn('Wing API Call Failed:', res);
            loadDemoOmsOrders(); // 실패 시 데모 폴백
            
            if (res && res.error && res.error.includes('KEYS_MISSING')) {
                if (typeof showToast === 'function') showToast('🚨 쿠팡 Wing API 키가 설정되지 않았습니다. T7 설정 탭을 확인하세요.', true, true);
            } else {
                if (typeof showToast === 'function') showToast('⚠️ 쿠팡 주문 수집에 실패하여 데모 데이터를 표시합니다.', true);
            }
        }
    } catch (e) {
        console.error('Order Fetch Error:', e);
        if (typeof showToast === 'function') showToast('❌ 주문 동기화 중 오류가 발생했습니다.', true, true);
        loadDemoOmsOrders();
    }
}
