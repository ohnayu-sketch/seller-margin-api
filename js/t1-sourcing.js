...
function renderBulkFilterBar() {
    ...
    el.innerHTML = `
        ...
        <div class="t1-bulk-actions">
            ...
            <button class="t1-ba-btn" style="color:var(--danger); border-color:var(--danger);" onclick="t1BulkDeleteChecked()">선택 일괄 삭제</button>
        </div>
    `;
}

// ─── 선택된 항목 일괄 삭제 (양방향 동기화) ───
async function t1BulkDeleteChecked() {
    const selected = T1.bulkItems.filter(i => i.checked);
    if (!selected.length) { showToast('삭제할 상품을 체크해주세요.', false); return; }

    if (!confirm(`선택한 ${selected.length}개의 상품을 일괄 삭제하시겠습니까?\n구글 시트에 연결된 상품의 경우 원본 시트 자체에서도 모두 삭제됩니다.`)) return;

    showLoading(true, `선택된 ${selected.length}개 상품 동기화 삭제 중... (최대 수십 초 소요될 수 있습니다)`);
    
    // 백엔드 삭제: 구글시트 연동 상품만 병렬 비동기 요청 시작
    const requests = selected.filter(i => i.originalId && String(i.originalId).startsWith('CLP-'))
                             .map(item => fetchGas('deleteProduct', { id: item.originalId }).catch(() => null));
    ...
    // 프론트엔드 삭제: 언체크된 것만 남김
    T1.bulkItems = T1.bulkItems.filter(i => !i.checked);
    T1.bulkItems.forEach((i, n) => { i.id = n + 1; });
    
    showLoading(false);
    showToast(`🗑️ ${selected.length}개 상품 삭제 완료.`);
...
function renderBulkTable() {
    ...
    filtered.forEach(item => {
        ...
        const marginClass = item.margin >= 25 ? 't1-c-green' : item.margin >= 15 ? 't1-c-warn' : 't1-c-red';
        const opacity = item.signal === 'GO' ? '1' : item.signal === 'WATCH' ? '0.6' : item.signal === 'PENDING' ? '0.9' : '0.35';
        ...
        html += `<tr style="opacity:${opacity}">
            ...
            <td>${item.link ? `<a href="${item.link}" target="_blank" style="color:inherit; text-decoration:underline;">${escapeHtml(item.name)}</a>` : escapeHtml(item.name)}</td>
            ...
            <td style="text-align:right">
                ...
                <button class="t1-mini-btn" style="background:var(--danger); border-color:var(--danger);" onclick="t1DeleteBulkItem(${item.id - 1})">삭제</button>
            </td>
        </tr>`;
    });
...
// ─── 대량 소싱 데이터 삭제 (양방향 동기화) ───
async function t1DeleteBulkItem(idx) {
    const item = T1.bulkItems[idx];
    if (!item) return;
    
    if (!confirm(`[ ${item.name} ]\n목록에서 완전히 삭제하시겠습니까?\n구글 시트 연동 상품인 경우 원본 시트에서도 삭제됩니다.`)) return;
    
    const isGoogleSheetItem = item.originalId && String(item.originalId).startsWith('CLP-');
    
    // 1. 프론트엔드 리스트에서 제거
    T1.bulkItems.splice(idx, 1);
    // 배열 순서에 맞게 ID 재정비
    T1.bulkItems.forEach((i, n) => { i.id = n + 1; });
    ...
    // 3. 구글 시트 원본 삭제 (양방향 동기화)
    if (isGoogleSheetItem) {
        showToast('🗑️ 구글 시트 연동 데이터를 삭제 중...');
        try {
            const res = await fetchGas('deleteProduct', { id: item.originalId });
            if (res && res.success) {
                showToast(`✅ "${item.name}" 구글 시트 완전 삭제 완료`);
            } else {
                showToast('❌ 구글 시트 삭제 실패 (수동 확인 필요)', false);
            }
        } catch(e) {
            console.error('deleteProduct Error:', e);
            showToast('❌ 구글 시트 네트워크 연결 오류', false);
        }
    } else {
        showToast('🗑️ 현재 보고 있는 화면 목록에서 제거되었습니다.');
    }
}
...
function t1ToggleAllChecked(checked) {
    T1.bulkItems.forEach(i => { i.checked = checked; });
    renderBulkFilterBar(); renderBulkTable();
}
...
async function t1ImportFromGoogleSheet() {
    ...
    try {
        ...
        if (productsArr?.length) {
            T1.bulkItems = productsArr.map((row, idx) => {
                let parsedCost = 0;
                const rawCost = row.cost || row.도매가 || row.원가 || 0;
                if (typeof rawCost === 'string') {
                    const cleaned = rawCost.replace(/[^0-9]/g, '');
                    parsedCost = parseInt(cleaned, 10);
                } else if (typeof rawCost === 'number') {
                    parsedCost = rawCost;
                }
                
                // 구글 시트 삭제 연동용 고유 ID 백업
                ...
                return {
                    ...
                    wholesalePrice: isNaN(parsedCost) ? 0 : parsedCost,
                    stock: '-', image: row.image || row.photoUrl || '',
                    source: row.market || row.source || '구글시트', 
