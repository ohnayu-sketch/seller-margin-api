...
function confirmSourcing(itemData) {
    const productData = {
        ...
        sourcing_type: itemData.sourcing_type || itemData.sourcingType || 'unknown', // ★ 위탁(drop) / 사입(bulk)
        source_url: itemData.wholesaleUrl || itemData.sourceUrl || itemData.url || '', // ★ 위탁/사입 링크 보존
        thumbnail_url: itemData.image || itemData.photoUrl || '',
        keyword: itemData.keyword || '',
        // ★ T1 -> T2 B2B 스펙 확장 파이프라인
    ...
    var typeLabel = productData.sourcing_type === 'drop' ? '🟢위탁' : (productData.sourcing_type === 'bulk' ? '📦사입' : '');
    if (typeof showToast === 'function') showToast(`✅ ${typeLabel} "${productData.name}" → T2(재고/사입)로 전달 완료`, 'success');
}
...
function sendToStudio(itemData) {
    ...
    if (typeof showToast === 'function') showToast(`🎬 "${studioData.name}" → T3 상세페이지 스튜디오로 전달`);
}
