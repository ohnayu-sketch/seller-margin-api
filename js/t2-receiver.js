...
window.renderT2Inventory = function() {
  ...
  // 수신부 컨테이너가 없으면 page-inventory 내부 메인 컨테이너에 생성
  if (!container) {
    container = document.createElement('div');
    container.id = 't2-sourcing-receiver';
    container.style.cssText = 'margin-bottom:20px; position:relative; z-index:10;'; // z-index 및 레이아웃 안정성 부여
    const mainWrap = page.querySelector('div') || page;
    mainWrap.prepend(container);
  }
