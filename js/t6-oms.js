...
function t6AddOrder() {
    ...
    const order = {
        ...
        sourcingType: matched?.sourcingType || 'unknown', // ★ 위탁(drop)/사입(bulk)
        status: 'received',
        trackingNo: '',
        trackingCompany: '',
        wholesaleOrderId: '',
...
function t6UpdateStatus(orderId, newStatus) {
    ...
    if (newStatus === 'settled') {
        ...
        AppEventBus.emit('ORDER_SETTLED', {
            ...
            market: order.market,
            sourcingType: order.sourcingType || 'unknown', // ★ 위탁/사입
...
function t6RenderOrders() {
    ...
    el.innerHTML = `<table class="t6-table">
        <thead><tr><th>상태</th><th>주문번호</th><th>상품명</th><th>소싱</th><th>마켓</th><th style="text-align:right">판매가</th><th style="text-align:right">수익</th><th>구매자</th><th>송장</th><th>다음단계</th><th>액션</th></tr></thead>
        <tbody>${items.map(o => {
            ...
            const stBadge = o.sourcingType === 'drop' ? '🟢위탁' : (o.sourcingType === 'bulk' ? '📦사입' : '-');
            const stColor = o.sourcingType === 'drop' ? '#4ade80' : (o.sourcingType === 'bulk' ? '#60a5fa' : '#94a3b8');
            return `<tr>
                ...
                <td><span style="font-size:9px;color:${stColor};font-weight:600">${stBadge}</span></td>
...
async function t6AutoOrder(orderId) {
    ...
    showLoading(true, `📦 "${order.productName}" ${order.sourcingType === 'drop' ? '도매매(위탁)' : '도매꾹(사입)'} 자동 발주 중...`);
    try {
