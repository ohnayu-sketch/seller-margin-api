...
async function fetchWholesaleApi(keyword, site = 'default') {
  ...
  // 응답 규격을 기존 { success, items: [...] } 형태로 맞춰줌
...
// 페이지 로드 시 hash 확인 (없을 경우 localStorage의 lastTab 복구 - 새로고침 탭 이탈 방지)
window.addEventListener('DOMContentLoaded', function() {
  ...
  if (hash && hash.startsWith('#tab-')) {
    ...
    // Hash가 없는 경우 (F5/새로고침 직후 등), 브라우저 로컬스토리지에 저장된 직전 탭을 띄웁니다.
    const lastTab = localStorage.getItem('lastTab');
    if (lastTab && ['sourcing','inventory','ledger','finance','studio','oms','setup'].includes(lastTab)) {
      setTimeout(() => { _showTabSkipPush = true; showTab(lastTab); }, 100);
    }
  }
});
...
async function runIntegratedV5Search() {
    ...
    try {
        ...
        try {
            const wsRes = await window.fetchGas('domeggookProxy', { type: 'search', keyword: keyword, market: 'dome', size: 20 });
            if (wsRes && wsRes.success) {
                wholesaleResult = { success: true, items: wsRes.data || wsRes.items || [] };
            }
        } catch(wsErr) {
            console.warn('도매꾹 API 통신 예외 — Mock 도매가 사용:', wsErr);
        }
