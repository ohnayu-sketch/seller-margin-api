...
function t1SetMode(mode) {
    T1.mode = mode;
    document.getElementById('t1-single-panel').style.display = mode === 'single' ? 'block' : 'none';
    document.getElementById('t1-bulk-panel').style.display = mode === 'bulk' ? 'block' : 'none';
...
// AI 소싱 스코어 계산
function calculateSourcingScore(data) {
    let score = 50; // 기본
    let reasons = [];

    // 가격 범위 (넓을수록 기회)
    const spread = data.maxPrice / Math.max(data.minPrice, 1);
    if (spread > 5) { score += 15; reasons.push('가격 범위 넓음(마진 기회↑)'); }
    else if (spread > 3) { score += 10; reasons.push('적정 가격 범위'); }

    // 상품 수 (많으면 수요 있음)
    if (data.totalCount >= 25) { score += 10; reasons.push('높은 시장 수요'); }
    else if (data.totalCount >= 15) { score += 5; reasons.push('보통 수요'); }

    // 최저가 (낮으면 도매가도 낮을 가능성)
    if (data.minPrice < 15000) { score += 5; reasons.push('저가 진입 가능'); }

    // 평균가 (2~5만원이 소싱 최적 구간)
    if (data.avgPrice >= 20000 && data.avgPrice <= 50000) { score += 10; reasons.push('최적 가격대(2~5만원)'); }

    score = Math.min(100, Math.max(0, score));
    const comment = reasons.slice(0, 3).join(', ') + (score >= 70 ? '. 소싱 추천!' : score >= 50 ? '. 추가 분석 권장.' : '. 신중한 검토 필요.');

    return { total: score, comment, reasons };
}
...
async function fetchKeywordIntelligence(keyword) {
    ...
    panel.innerHTML = '<div class="t1-loading-inline">🔍 키워드 분석 중...</div>';

    try {
        const result = await fetchGas('naverAdProxy', { keyword: keyword });
        if (!result?.success || !result.keywords?.length) {
            panel.innerHTML = `<div class="t1-section-header"><span>📊 키워드 인텔리전스</span><span class="t1-badge-new">NEW</span></div>
                <div class="t1-info-msg">💡 네이버 검색광고 API 키를 T7 설정에 등록하면 월간검색량/경쟁강도를 분석합니다.</div>`;
            return;
        }

        const main = result.keywords.find(k => k.keyword === keyword) || result.keywords[0];
        const related = result.keywords.filter(k => k.keyword !== keyword).sort((a,b) => b.monthlyTotal - a.monthlyTotal).slice(0, 10);

        let html = `<div class="t1-section-header"><span>📊 키워드 인텔리전스</span><span class="t1-badge-new">NEW</span></div>`;

        // 메인 키워드 요약
        html += `<div class="t1-kw-summary">
            <div class="t1-kw-stat"><div class="t1-kw-val t1-c-green">${fmt(main.monthlyTotal || 0)}</div><div class="t1-kw-label">월간 검색량</div></div>
            <div class="t1-kw-stat"><div class="t1-kw-val" style="color:${getCompetitionColor(main.competitionIdx)}">${main.competitionIdx || '-'}</div><div class="t1-kw-label">경쟁 강도</div></div>
            <div class="t1-kw-stat"><div class="t1-kw-val">${getSourcingGrade(main.monthlyTotal, main.competitionIdx)}</div><div class="t1-kw-label">소싱 추천도</div></div>
        </div>`;
        ...
        if (related.length) {
            ...
            related.forEach((kw, i) => {
                html += `<div class="t1-kw-row" onclick="document.getElementById('t1-search-input').value='${kw.keyword}';t1SingleSearch();">
                    ...
                    <span class="t1-kw-vol">${fmt(kw.monthlyTotal)}</span>
                    <span class="t1-kw-comp" style="color:${getCompetitionColor(kw.competitionIdx)}">${kw.competitionIdx || '-'}</span>
                    <span class="t1-kw-grade">${getSourcingGrade(kw.monthlyTotal, kw.competitionIdx)}</span>
                </div>`;
            });
            html += `</div>`;
        }
...
function getCompetitionColor(idx) {
    if (idx === '높음') return '#ef4444';
    if (idx === '중간') return '#f59e0b';
    return '#10b981';
}

function getSourcingGrade(monthly, comp) {
    const vol = parseInt(monthly) || 0;
    if (vol > 5000 && comp === '낮음') return '🏆 최적';
    if (vol > 3000 && comp !== '높음') return '👍 양호';
